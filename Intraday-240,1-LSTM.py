...
all_companies = [company for company in all_companies if pd.notnull(company)]  # Remove nan
...
for test_year in range(1993, 2016):
    months = [str(t)+'-0'+str(m) if m < 10 else str(t)+'-'+str(m) 
              for t in range(test_year-3, test_year) for m in range(1, 12)]
    constituents_train[test_year] = [list(constituents[m]) for m in months if m in constituents]
...
def trainer(train_data, test_data, model_type='LSTM'):
    ...
    # Check and handle NaN values in train_x, train_y, train_ret
    if np.isnan(train_x).any() or np.isnan(train_y).any() or np.isnan(train_ret).any():
        train_x = np.nan_to_num(train_x)
        train_y = np.nan_to_num(train_y)
        train_ret = np.nan_to_num(train_ret)
...
for test_year in range(1993, 2020):
    ...
    # Check for NaNs in the data using pandas
    train_data = pd.DataFrame(train_data)
    test_data = pd.DataFrame(test_data)
    
    if train_data.isnull().values.any() or test_data.isnull().values.any():
        print(f"NaNs detected in data for year {test_year}. Handling NaNs...")
        train_data = train_data.fillna(0).values
